...
JSON_PATH_TEMPLATE = "Output/consolidated_reasoning_{}.json"  # file per session
...
class StoryApp:
    def __init__(self, root, facilitator_id):  # Pass facilitator_id here!
        self.root = root
        self.root.title("Jira Story Facilitation Tool")
        self.root.configure(bg="#e3eafc")
        self.session_id = str(uuid.uuid4())
        self.data_json_path = f"Output/consolidated_reasoning_{self.session_id}.json"
    ...
    def submit_story(self):
        ...
        # Remove this key from the dropdown so it can't be resubmitted in this session
        keys_left = [v for v in self.jira_key_combo['values'] if v != key]
        self.jira_key_combo['values'] = keys_left
        self.jira_key_var.set("")
        self.reset_details_and_inputs()
        # Optionally: after all keys are submitted, disable submit button
        if not keys_left:
...
def main():
    root = tk.Tk()
    root.withdraw()  # Hide root window during facilitator prompt
    ...
    root.deiconify()  # Show the root window after successful facilitator login
    app = StoryApp(root, facilitator_id)  # Pass facilitator_id to app!
    root.mainloop()
